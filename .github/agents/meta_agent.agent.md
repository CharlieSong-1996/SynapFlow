...
name: Meta Agent
tools: [agent, read/readFile, edit/createFile, edit/createDirectory, edit/editFiles, web/fetch, search/listDirectory, search/changes, vscode/askQuestions, vscode/extensions, vscode/vscodeAPI, browser/openBrowserPage ]
...
9. After creating/editing agent file, check using Agent Validator subAgent, if there is any issue, report to user and ask for confirmation to fix the issue.
...
7. If user requst you to check a agent file, check with Agent Validator subAgent, instead of checking by yourself.
